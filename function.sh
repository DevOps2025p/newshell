...
then
    ...
    echo "Script is running with root user" | tee -a $LOG_FILE
fi
...
VALIDATE() {
    if [ $1 -eq 0 ]
    then
        echo "Installing $2 is successful" | tee -a $LOG_FILE
    else
...
if [ $? -ne 0 ]
then
    echo "Python3 is not installed, installing now" | tee -a $LOG_FILE
...
if [ $? -ne 0 ]
then
    echo "Nginx is not installed, installing now" | tee -a $LOG_FILE
